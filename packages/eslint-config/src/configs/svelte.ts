...
export async function svelte(
	...
	return [
		...
		{
			files: [GLOB_SVELTE],
			name: 'kp/svelte/component',
			processor: 'svelte/svelte',
			rules: {
				// Required by eslint-plugin-svelte's base component configuration.
		...
		{
			files: ['**/routes/**/+*.ts'],
			name: 'kp/svelte/sveltekit-routes',
			rules: {
				// Error helpers imported from '@sveltejs/kit' are intentionally thrown.
